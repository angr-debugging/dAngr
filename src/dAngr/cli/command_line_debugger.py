...
from dAngr.cli.debugger_commands import CommandSpec
...
def _auto_register_commands() -> Dict[str, CommandSpec]:
...
class CommandLineDebugger(Debugger,StepHandler):
    ...
    async def handle(self, command:str):
        try:
            ...
            if command_name == "help" or command_name == "?":
                ...
                if command_args:
                    spec = self._get_command_spec(command_args)
                    if spec is None:
                        await self.conn.send_error(f"Command '{command_args}' not found. Type 'help' or '?' for a list of available commands.")
                    else:
                        await self._list_args(spec)
                else:
                    await self._list_commands()
            else:
                # Execute the command
                spec = self._get_command_spec(command_name)
                if spec is None:
    ...
    def list_args_table(self, spec :CommandSpec,indent = []) -> Tuple[List[List[str]],Style]:
        style = Style.from_dict({
            'title': '',
            'command': 'blue bold',
            'info': '',
        ...
        required = spec.get_required_args()
        optional = spec.get_optional_args()
        ...
        if spec.args:
            ...
            if optional:
                ...
                for a in optional:
                    if a.type:
                        if members:= get_union_members(a.type):
                            tp = " or ".join([m.__name__ for m in members])
                        else:
                            tp = a.type.__name__
                    else: tp ="any"
    ...
    async def _list_args(self, spec :CommandSpec):
    ...
    def _parse_arguments(self,spec :CommandSpec, user_input:str)->List: 
